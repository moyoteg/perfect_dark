#!/usr/bin/env bash
# Deploy matrix_battle_sentry (uff test slot) and launch with laptop sentry toggles.
# Usage: ./scripts/play-matrix-laptop-sentry.sh [--no-play] [--num-sims N]
set -euo pipefail
...
LAST_LOG="$LOG_DIR/.matrix_battle_sentry_test.log"
NUM_SIMS=50
NO_PLAY=false
...
echo "=== Matrix Battle Sentry (Laptop Gun) ==="
echo "  Deploy: python3 tools/pdmap.py build matrix_battle_sentry --seg --deploy"
echo "  Binary: ${PD#$REPO/}"
echo "  Flags:  --laptop-sentry-x4 --unlimited-sentries"
...
python3 tools/pdmap.py build matrix_battle_sentry --seg --deploy
...
{
	echo "=== $(date -u +%Y-%m-%dT%H:%M:%SZ) matrix_battle_sentry laptop sentry test ==="
...
APPLESCRIPT
	...
	pgrep_play() {
		...
		for p in $(pgrep -x pd.arm64 2>/dev/null); do
			...
			if [[ "$cmd" == *"$PD"* && "$cmd" == *"--test-map"* && "$cmd" == *"--laptop-sentry-x4"* ]]; then
				echo "$p"
				return 0
			fi
		done
...
APPLESCRIPT
		osascript -e "display notification \"Laptop sentry x4 + unlimited deploys.\" with title \"Perfect Dark — Matrix Sentry\"" >/dev/null 2>&1 || true
	fi
