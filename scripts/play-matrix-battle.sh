#!/usr/bin/env bash
# Deploy matrix_battle_64 into the uff test slot and launch a long teams Combat match.
# Usage: ./scripts/play-matrix-battle.sh [--no-play] [--num-sims N]
set -euo pipefail
...
LAST_LOG="$LOG_DIR/.matrix_battle_64_test.log"
NUM_SIMS=50
NO_PLAY=false
...
echo "=== Matrix Battle 64 ==="
echo "  Deploy: python3 tools/pdmap.py build matrix_battle_64 --seg --deploy"
echo "  Binary: ${PD#$REPO/}"
...
python3 tools/pdmap.py build matrix_battle_64 --seg --deploy
...
{
	echo "=== $(date -u +%Y-%m-%dT%H:%M:%SZ) matrix_battle_64 teams Combat (play-matrix-battle) ==="
...
APPLESCRIPT
	...
	pgrep_play() {
		...
		for p in $(pgrep -x pd.arm64 2>/dev/null); do
			...
			if [[ "$cmd" == *"$PD"* && "$cmd" == *"--test-map"* && "$cmd" == *"--teams-battle"* ]]; then
				echo "$p"
				return 0
			fi
		done
...
APPLESCRIPT
		osascript -e "display notification \"Matrix Battle 64 — 60 min, no score cap (teams Combat).\" with title \"Perfect Dark — Matrix Battle\"" >/dev/null 2>&1 || true
	fi
